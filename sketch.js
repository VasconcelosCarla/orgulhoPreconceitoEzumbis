...
var explosionBallAnimation = [];
var explosionBallSpritedata, explosionBallSpritesheet;
...
function preload() {
  ...
  backgroundMusic = loadSound("./assets/DragonCastle.mp3");
  cannonSound = loadSound("./assets/shot.mp3");
  towerImage = loadImage("./assets/tower.png");
  gramaImagem = loadImage("./assets/grama2.png");
  
  ballAnimation = [loadImage("./assets/cannonball.png")];
  ...
  explosionBallSpritedata = loadJSON("./assets/cannonBall2/cannonBall2.json");
  explosionBallSpritesheet = loadImage("./assets/cannonBall2/cannonBall2.png");
...
function loadAnimationFrames() {
  ...
  var explosionFrames = explosionBallSpritedata.frames;
  for (var i = 0; i < explosionFrames.length; i++) {
    var pos = explosionFrames[i].position;
    var img = explosionBallSpritesheet.get(pos.x, pos.y, pos.w, pos.h);
    explosionBallAnimation.push(img);
  }
}
  ...
  for (var i = 0; i < invaders.length; i++) {
    if (balls[index] !== undefined && invaders[i] !== undefined) {
      ...
      if (collision.collided) {
        ...
        balls[index].remove(index);  // Remover a bola de canhão corretamente
      }
    }
  }
}
  ...
  if (invaders.length > 0) {
    ...
    for (var i = 0; i < invaders.length; i++) {
      if (invaders[i]) {
        ...
        invaders[i].display();
        invaders[i].animate();
        var collision = Matter.SAT.collides(tower, invaders[i].body);
        if(collision.collided && !invaders[i].isBroken){
          isGameOver = true;
          gameOver();
        }
      }
...
function displayTower() {
  ...
  rotate(tower.angle);
  imageMode(CENTER);
  image(towerImage, 0, 0, 160, 350);
  pop();
}

function displayGrama() {
  push();
...
function gameOver(){
  swal.fire({
    ...
    imageUrl:"https://github.com/VasconcelosCarla/orgulhoepreconceito/blob/main/assets/Idle1.png",
    imageWidth: 150,
    imageHeight:150,
